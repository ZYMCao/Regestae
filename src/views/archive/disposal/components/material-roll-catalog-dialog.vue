<template>
  <!-- 添加案卷目录弹窗（编研素材）：左侧档案分类目录 + 右侧条件查询与案卷列表（静态演示数据） -->
  <el-dialog
    :visible.sync="dialogVisible"
    :fullscreen="fullscreen"
    :width="fullscreen ? '100%' : '92%'"
    top="4vh"
    custom-class="material-roll-catalog-dialog"
    append-to-body
  >
    <template slot="title">
      <div class="mrc-header">
        <span class="mrc-title">添加案卷目录</span>
        <i class="el-icon-crop mrc-fullscreen" @click="fullscreen = !fullscreen" />
      </div>
    </template>
    ...
    <div class="mrc-body">
      ...
      <div class="mrc-tree">
        ...
        >
          <span slot-scope="{ data }" class="tree-node">
            <i :class="['el-icon-folder', 'node-icon', { sub: data.sub }]" />
      ...
      <div class="mrc-main">
        ...
        <div v-show="expandQuery" class="filter-area">
          <el-form :model="filters" size="small" label-width="90px">
            <el-form-item label="案卷题名">
              <el-input v-model="filters.title" placeholder="请输入案卷题名" />
            </el-form-item>
            <el-row>
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-form-item label="编制单位">
                  <el-input v-model="filters.unit" placeholder="请输入编制单位" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="编制起日期">
                  <el-date-picker v-model="filters.startDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" style="width: 100%" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="编制止日期">
                  <el-date-picker v-model="filters.endDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" style="width: 100%" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-checkbox v-model="filters.includeChild">包含子节点目录</el-checkbox>
            <el-checkbox v-model="filters.grouped">已组卷</el-checkbox>
          </el-form>
        </div>
        ...
        <el-table :data="rows" border size="small" height="340" @selection-change="s => (selection = s)">
          ...
          <el-table-column label="检查情况" prop="checkInfo" width="110" sortable show-overflow-tooltip />
          <el-table-column label="案卷题名" prop="title" min-width="220" sortable show-overflow-tooltip />
          <el-table-column label="档号" prop="archiveNo" width="140" sortable show-overflow-tooltip />
          <el-table-column label="编制单位" prop="unit" min-width="180" sortable show-overflow-tooltip />
          ...
          <el-table-column label="保管期限" prop="term" width="100" align="center" sortable />
        </el-table>
...
<script>
// 档案分类目录（静态演示；sub: true 表示三级分类节点，图标为蓝色）
const TREE_DATA = [
  {
    id: 1,
    name: '柯桥至诸暨高速公路工程',
    children: [
      {
        ...
        name: '第一部分 项目申报文件',
        children: [
          { id: 111, name: '一、项目建议书及批复、申报文件', sub: true },
          { id: 112, name: '二、可行性研究报告及批复文件', sub: true },
          { id: 113, name: '三、环境影响报告及批复文件', sub: true },
          { id: 114, name: '四、水土保持方案报告及批复', sub: true },
          { id: 115, name: '五、项目咨询、评估、论证材料', sub: true },
          { id: 116, name: '六、投资、特许经营协议', sub: true },
          { id: 117, name: '七、工程融资贷款计划、批复', sub: true }
        ]
      },
...
export default {
  name: 'MaterialRollCatalogDialog',
  props: {
    visible: {
      type: Boolean,
      default: false
  ...
  watch: {
    visible(val) {
      if (val) {
        ...
        this.$nextTick(() => {
          // 默认选中"一、项目建议书及批复"节点（与截图一致）
          this.$refs.tree && this.$refs.tree.setCurrentKey(111)
        })
      }
    }
  },
  methods: {
    ...
    // 生成演示案卷数据（以当前选中分类为题名前缀）
    buildDemoRows() {
      const nodeName = this.currentNode ? this.currentNode.name : '第一部分 项目申报文件'
      const rows = []
      for (let i = 1; i <= 8; i++) {
        const term = i % 3 === 0 ? '定期10年' : i % 3 === 1 ? '定期30年' : '永久'
        rows.push({
          id: i,
          checkInfo: i % 2 === 0 ? '已检验' : '未检验',
          title: nodeName + '案卷' + i,
          archiveNo: 'KZGS-' + (1000 + i * 7),
          unit: '绍兴市柯诸高速公路有限公司',
          startDate: '2025-0' + ((i % 6) + 1) + '-15',
          endDate: '2026-0' + ((i % 6) + 1) + '-15',
          term: term,
          secret: i % 2 === 0 ? '内部' : '公开'
        })
      }
