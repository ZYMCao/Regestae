<template>
  <!-- 添加案卷目录弹窗：左侧档案分类目录 + 右侧条件查询与案卷列表（静态演示数据） -->
  <el-dialog
    :visible.sync="dialogVisible"
    :fullscreen="fullscreen"
    :width="fullscreen ? '100%' : '92%'"
    top="4vh"
    custom-class="roll-catalog-select-dialog"
    append-to-body
  >
    <template slot="title">
      <div class="rcs-header">
        <span class="rcs-title">添加案卷目录</span>
        <i class="el-icon-crop rcs-fullscreen" @click="fullscreen = !fullscreen" />
      </div>
    </template>
    ...
    <div class="rcs-body">
      ...
      <div class="rcs-main">
        ...
        <el-table :data="rows" border size="small" height="340" @selection-change="s => (selection = s)">
          ...
          <el-table-column label="考证起日期" prop="checkStartDate" width="110" align="center" sortable />
        </el-table>
    ...
    buildDemoRows() {
      ...
      for (let i = 1; i <= 8; i++) {
        ...
        rows.push({
          ...
          checkStartDate: '2026-0' + ((i % 4) + 1) + '-01',
          term: term,
          secret: i % 2 === 0 ? '内部' : '公开'
        })
      }
