<template>
  <!-- 添加文件目录弹窗（编研素材）：左侧档案分类目录 + 右侧条件查询与文件列表（静态演示数据） -->
  <el-dialog
    :visible.sync="dialogVisible"
    :fullscreen="fullscreen"
    :width="fullscreen ? '100%' : '92%'"
    top="4vh"
    custom-class="material-file-catalog-dialog"
    append-to-body
  >
    <template slot="title">
      <div class="mfc-header">
        <span class="mfc-title">添加文件目录</span>
        <i class="el-icon-crop mfc-fullscreen" @click="fullscreen = !fullscreen" />
      </div>
    </template>
    ...
    <div class="mfc-body">
      ...
      <div class="mfc-main">
        ...
        <el-table :data="rows" border size="small" height="340" @selection-change="s => (selection = s)">
          ...
          <el-table-column label="件号" prop="itemNo" width="90" align="center" sortable />
...
export default {
  name: 'MaterialFileCatalogDialog',
  props: {
    visible: {
      type: Boolean,
      default: false
    ...
    buildDemoRows() {
      ...
      for (let i = 1; i <= 8; i++) {
        ...
        rows.push({
          id: i,
          title: nodeName + '文件' + i,
          archiveNo: 'KZGS-' + (2000 + i * 5),
          itemNo: i,
          docNo: '绍柯诸高速〔2025〕' + i + '号',
          author: '绍兴市柯诸高速公路有限公司',
          createDate: '2025-0' + ((i % 6) + 1) + '-20',
          term: term,
          secret: i % 2 === 0 ? '内部' : '公开'
        })
      }
